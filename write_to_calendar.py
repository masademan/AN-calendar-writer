...
def create_calendar_event(event_name, start_datetime, end_datetime, day_of_week_repeat, repeat_until_datetime, verbosity=2):
    ...
    event_body = {
        ...
        "recurrence": [f"RRULE:FREQ=WEEKLY;BYDAY={day_of_week_repeat};UNTIL={repeat_until_datetime}"],
        "reminders": {
            "useDefault": True,
        },
    }
    ...
    try:
        ...
        created_event = service.events().insert(calendarId=constants.CLASS_CALENDAR_ID, body=event_body).execute()

        if verbosity == 2:
            print(f"Success! Event '{event_name}' created")
