...
def delete_calendar_event(event_name_to_delete, verbosity=2, time_range: tuple[datetime, datetime] | None = None):
    ...
    try:
        if verbosity == 2:
            print(f"Searching for events matching: '{event_name_to_delete}'...")
        ...
        for event in events:
            ...
            if event.get("summary") == event_name_to_delete:
                event_id = event["id"]
                if verbosity == 2:
                    print(f"Found exact match: {event.get('summary')} (ID: {event_id})")

                # 4. Delete the event using its ID
                service.events().delete(calendarId=constants.CLASS_CALENDAR_ID, eventId=event_id).execute()

                if verbosity == 2:
                    print("Success! Event and all its repeating instances have been deleted.")

                # Stop after deleting the first exact match to avoid deleting duplicates
                return

        if verbosity == 2:
            print("Found some search results, but none matched that exact title.")
...
def clear_day_of_events(target_date, verbosity=2):
    ...
    try:
        if verbosity == 2:
            print(f"Searching for all events on {target_date}...")
        ...
        if verbosity == 2:
            print(f"Found {len(events)} event(s). Deleting them...")
        ...
        if verbosity == 2:
            print("Success! All events on this day have been removed.")
