...
def convert_time(time, days, is_24_hr=False, IANA_timezone=None):
    now = datetime.now()
    if not IANA_timezone: local_tz = now.astimezone().tzinfo
    else: local_tz = ZoneInfo(IANA_timezone)
    ...
    if not is_24_hr: time = convert_to_24h(time)
...
def get_class_data_file_name(school_year=None):
    ...
    if school_year == None: school_year_str = input("School starting year: ").lstrip().rstrip()
    else: school_year_str = str(school_year)
